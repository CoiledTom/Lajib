...
-- ── Salvar / Carregar key (_G — sem arquivo no celular) ───────────────────
_G.SystemUI_PandaKeys = _G.SystemUI_PandaKeys or {}

local function SaveKey(key)
    _G.SystemUI_PandaKeys[SERVICE_ID] = key
    pcall(writefile, SAVE_FILE, key)  -- bonus: grava também se executor suportar
end

local function LoadKey()
    local gk = _G.SystemUI_PandaKeys[SERVICE_ID]
    if gk and gk ~= "" then return gk end
    local ok, v = pcall(readfile, SAVE_FILE)
    return (ok and v and v ~= "") and v or nil
end

-- ── Validação PandAuth (POST com API Key no header) ────────────────────────
local function ValidatePanda(key)
    local headers = { ["Content-Type"] = "application/json" }
    if API_KEY ~= "" and API_KEY ~= "SUA-API-KEY-AQUI" then
